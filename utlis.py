...
def colorFilter(img):
    ...
    lowerYellow = np.array([144,238,144])
...
def initializeTrackbars(intialTracbarVals):
    ...
    cv2.createTrackbar("Width Top", "Trackbars", intialTracbarVals[0], 40, nothing)
    cv2.createTrackbar("Height Top", "Trackbars", intialTracbarVals[1], 80, nothing)
    cv2.createTrackbar("Width Bottom", "Trackbars", intialTracbarVals[2], 50, nothing)
    cv2.createTrackbar("Height Bottom", "Trackbars", intialTracbarVals[3], 80, nothing)
...
def valTrackbars():
    ...
    src = np.float32([(widthTop / 99, heightTop / 97), (1.02- (widthTop / 97), heightTop / 97),
                      (widthBottom / 97, heightBottom / 97), (1.02 - (widthBottom / 97), heightBottom / 97)])
    return src
